...
class BlockchainController {
    ...
    func getTransactions(req: Request) throws -> [Transaction] {
        guard let drivingLicenseNumber = req.parameters.get("drivingLicenseNumber") else {
            throw Abort(.notFound)
        }
        return blockchainService.blockchain.transactionBy(drivingLicenseNumber: drivingLicenseNumber)
